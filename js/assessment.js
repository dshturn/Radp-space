...
async function generateLoR() {
  ...
  const downloadBtn = `<div style="margin-top:12px;"><button onclick="downloadLoRPDF()" style="background:#1e3a5f;color:white;border:none;padding:7px 18px;border-radius:6px;cursor:pointer;font-size:11px;margin-right:8px;">⬇️ Download as PDF</button><button onclick="window.print()" style="background:#666;color:white;border:none;padding:7px 18px;border-radius:6px;cursor:pointer;font-size:11px;">🖨️ Print</button></div>`;
  const htmlWithBtn = lorHtml.replace('</body>', `${downloadBtn}</body>`);
  const newBlob = new Blob([htmlWithBtn], { type: 'text/html' });
  const newBlobUrl = URL.createObjectURL(newBlob);
  window.open(newBlobUrl, '_blank');
  window._currentLoRHtml = lorHtml;
  setTimeout(() => { URL.revokeObjectURL(blobUrl); URL.revokeObjectURL(newBlobUrl); }, 60000);
}

async function downloadLoRPDF() {
  if (!window._currentLoRHtml) { showToast('No LoR data available', 'error'); return; }
  showToast('Converting to PDF...', 'info');

  try {
    const apiUrl = typeof window !== 'undefined' && window.location.port === '3001'
      ? 'http://localhost:5000/api/generate-html-pdf'
      : '/api/generate-html-pdf';
    ...
    const response = await fetch(apiUrl, {
      ...
      body: JSON.stringify({ html: window._currentLoRHtml })
    });

    if (!response.ok) {
      const err = await response.json().catch(() => ({}));
      showToast('PDF conversion failed: ' + (err.error || response.status), 'error');
      return;
    }
    ...
    link.href = url;
    link.download = `LoR_${new Date().toISOString().split('T')[0]}.pdf`;
