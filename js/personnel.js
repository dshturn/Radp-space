...
async function loadPersonnel(preserveState = false) {
  ...
  const persSelect = isAdmin ? 'select=*,user_profiles!contractor_id(company)' : 'select=*';
  const res  = await fetch(
    `${SUPABASE_URL}/rest/v1/personnel?${persSelect}&order=created_at${_persSearch ? `&full_name=ilike.*${encodeURIComponent(_persSearch)}*` : ''}&offset=${from}&limit=${_PERS_PAGE_SIZE}`,
  ...
  return `<div class="app-card" data-id="p${parseInt(p.id)}">
    ...
    <div class="card-header">
      <div class="card-clickable" onclick="toggleCard(this.closest('.app-card').querySelector('.btn-toggle'))">
        ...
        ${isAdmin && p.user_profiles?.company ? `<div class="doc-name mt-xs" style="color:var(--text-3);font-size:11px;">${esc(p.user_profiles.company)}</div>` : ''}
      </div>
      <div class="row-wrap">
        ${alertBadge}
