...
function personnelCard(p, docs, isAdmin = false) {
  ...
  const docRows = PERS_DOC_TYPES.map(t => {
    ...
    if (d) {
      ...
      return `<div class="doc-row">
        ...
        <button class="upload-btn" onclick="openAddPersDoc(${parseInt(p.id)}, this.dataset.name, this.dataset.type, ${t.mandatory ? 'true' : 'false'})" data-type="${safeTypeName}" data-name="${esc(p.full_name)}">↑ Upload</button>
      </div>`;
    }
  }).join('');
  ...
  return `<div class="app-card" data-id="p${parseInt(p.id)}">
    ...
    <div class="card-header">
      ...
      <div class="row-wrap">
        ${alertBadge}
        ${isAdmin ? `<button class="btn-edit" onclick="openEditPersonnel(${parseInt(p.id)})" aria-label="Edit ${safeName}" style="padding:6px 10px;border:1px solid var(--border);border-radius:4px;background:var(--surface-2);color:var(--text-1);cursor:pointer;font-size:12px;">Edit</button>` : ''}
        <button class="btn-toggle" onclick="toggleCard(this)" aria-label="Expand ${safeName}" aria-expanded="false">▾</button>
...
async function deletePersDoc(id) {
  ...
  const persCard = el?.closest('.sub-card-body,.card-body');
  const persName = persCard?.querySelector('.card-title')?.textContent || 'Personnel';
  const label = `${persName} - ${docName}`;
