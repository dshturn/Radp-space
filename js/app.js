...
const ROLE_NAV     = {
  ...
  admin:      new Set(['contractor', 'assessment', 'operations', 'users', 'audit', 'notifications']),
};
...
function showPage(name, replace = false) {
  ...
  if (NAV_PAGES.has(name)) {
    ...
    const navMap = { contractor: 'navContractor', assessment: 'navAssessment', operations: 'navOperations', users: 'navUsers', audit: 'navAudit', notifications: 'navNotifications' };
    ...
    if (name === 'contractor') {
      ...
      const tabs = document.querySelectorAll('#contractor-page .tabs .tab');
  ...
  if (name === 'users')      { adminInit(); }
  if (name === 'audit')      { auditInit?.(); }
