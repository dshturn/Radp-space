...
async function refreshActiveRequests() {
  try {
    ...
    const preloadedRecords = await Promise.all(
      activeRecords.map(item => renderRecord(item, 'Active Requests'))
    );

    // Hide loading screen and render
    document.getElementById('loading-screen').style.display = 'none';
    renderActiveRequests(preloadedRecords, 'Active Requests');
...
const url = `https://sharek.aramco.com.sa/orgs/30002972/30037952/_api/web/lists/GetByTitle('${listTitle}')/fields?$filter=substringof('Focal Point',Title)`;
  fetch(url, {
    method: 'GET',
    headers: {
      'Accept': 'application/json; odata=verbose'
  ...
  .then(data => {
    const columnName = data.d.results[0].InternalName;

    const versionUrl = `https://sharek.aramco.com.sa/orgs/30002972/30037952/_api/web/lists/GetByTitle('${listTitle}')/items(${record.record.Id})/versions?$select=UniqueId,VersionId,Modified,Editor/Id,Editor/Name,Status&$expand=Editor`;
    fetch(versionUrl, {
      method: 'GET',
      headers: {
        'Accept': 'application/json; odata=verbose'
    ...
    .then(data => {
      ...
      versionHistory.forEach((version) => {
        const existingVersion = uniqueVersions.find((v) => {
          return v.status === version.status &&
      ...
      const requestInfoUrl = `https://sharek.aramco.com.sa/orgs/30002972/30037952/_api/web/lists/GetByTitle('${listTitle}')/items(${record.record.Id})?$select=Created,Title,Service_x0020_Provider,Service_x0020_Type,${columnName},FileLeafRef`;
...
idRow.appendChild(idValueCell);
        ...
        focalPointValueCell.textContent = requestInfo[columnName];
...
let progress = 0;
const totalRecords = activeRecords.length + completedRecords.length + onwcodRecords.length + offwcodwsdRecords.length + onwsdRecords.length;
...
// Update progress value every second
...
document.getElementById('refresh-button').addEventListener('click', async () => {
  try {
    ...
    // Flatten and map records with list index
    const allData = data.flatMap((item, index) =>
      item.d.results.map(record => ({
        record,
        listIndex: index
      }))
    );
    ...
    // Filter and map records based on current tab
    let recordsToRender;
    if (currentTabName === 'Active Requests') {
      recordsToRender = allData
        .filter(item => !item.record.Status?.toLowerCase?.().includes('completed'))
        .map(item => ({
          Title: item.record.Title,
          ListTitle: listNames[item.listIndex],
          record: item.record
      ...
      if (matchedList) {
        recordsToRender = allData
          .filter(item => listNames[item.listIndex] === matchedList)
          .map(item => ({
            Title: item.record.Title,
            ListTitle: listNames[item.listIndex],
            record: item.record
    ...
    // Sort records by Modified date (descending)
    ...
    document.addEventListener('click', (e) => {
          ...
          async function getStatusChoices(listTitle) {
            try {
              ...
              if (fieldResponse.ok) {
                ...
                console.log(`Failed to retrieve the "Status" field from the "${listTitle}" list. Status code: ${fieldResponse.status}`);
                return [];
              }
            } catch (error) {
                ...
                async function updateRecordStatus(record, newListTitle, newStatus, lorExcelFile, lorPdfFile) {
                  try {
                    ...
                    if (updateResponse.ok) {
                      ...
                      const preloadedData = {
                        'Active Requests': await Promise.all(activeRecords.map((record, index) => {
                          return renderRecord(record, 'Active Requests');
                        })),
                      };
      ...
      // Refresh the active requests table
    ...
    alert('Error updating record. Please try again.');
                ...
                await updateRecordStatus(record, record.ListTitle, newStatus, lorExcelFile, lorPdfFile);
              };
