...
app.post('/api/generate-lor-pdf', async (req, res) => {
  ...
  try {
    ...
    const lorHtml = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>LoR</title><style>body{font-family:Arial;font-size:10px;margin:10px}table{border-collapse:collapse;width:100%;margin:10px 0}td{border:1px solid #bbb;padding:4px;vertical-align:top}td.ac{text-align:center}h2{margin:5px 0}h3{margin:10px 0 5px 0;font-size:12px}p{margin:3px 0;font-size:10px}</style></head><body><h2>List of Readiness</h2><p>Assessment: <strong>${assessment.id}</strong> | <strong>${esc(assessment.company_name||'—')}</strong> | Field: ${esc(assessment.field_well||'—')} | ${todayStr}</p><h3>Personnel</h3><table><tr style="background:#1e3a5f;color:white;font-weight:bold;"><td style="width:5%">No.</td><td style="width:20%">Name</td><td style="width:10%">Exp.</td><td style="width:15%">Position</td><td style="width:20%">Documents</td><td style="width:15%">Expiry Date</td><td style="width:5%">✓</td><td style="width:5%">✗</td><td style="width:5%">Notes</td></tr>${persRows}</table><h3>Equipment</h3><table><tr style="background:#2d4a1e;color:white;font-weight:bold;"><td style="width:5%">No.</td><td style="width:15%">Serial</td><td style="width:25%">Equipment</td><td style="width:15%">Document</td><td style="width:10%">Issue Date</td><td style="width:15%">Expiry Date</td><td style="width:5%">✓</td><td style="width:5%">✗</td></tr>${equipRows}</table></body></html>`;
